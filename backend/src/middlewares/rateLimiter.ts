 ...
 * Falls back to IP using express-rate-limit's built-in key generator.
 * Uses validate: false to suppress IPv6 validation errors on custom keyGenerators.
 */
const getUserKey = (req: Request): string => {
  return (req as any).user?.id || 'anon';
};
