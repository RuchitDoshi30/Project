          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className={`text-xs px-2 py-0.5 rounded font-medium ${getDifficultyColor(problem.difficulty)}`}>
                  {problem.difficulty}
                </span>
              </div>
                  ...
                  <div
                    ...
                    dangerouslySetInnerHTML={{
                      __html: problem.description
                        .replace(/`([^`]+)`/g, '<code class="px-1.5 py-0.5 bg-gray-100 rounded text-sm font-mono text-gray-900">$1</code>')
                        .replace(/\*\*([^*]+)\*\*/g, '<strong>$1</strong>')
                        .replace(/\n\n/g, '</p><p class="mt-4">')
                        .replace(/^```([\s\S]*?)```/gm, '<pre class="bg-gray-900 text-gray-100 p-4 rounded-lg overflow-x-auto"><code>$1</code></pre>'),
                    }}
                  />
                    ...
                    !testCase.isHidden && (
                      <Card key={idx} className="p-4">
                        <div className="flex items-start justify-between mb-2">
                          <h4 className="text-sm font-semibold text-gray-900">Example {idx + 1}</h4>
                        </div>
                  ...
                  {submissions.length === 0 ? (
                    ...
                    submissions.map((sub) => (
                      <Card key={sub._id} className="p-4 hover:shadow-md transition-shadow">
                        <div className="flex items-start justify-between mb-2">
                          <div className="flex items-center gap-2">
                            <span className={`text-xs px-2 py-1 rounded font-medium ${getStatusColor(sub.status)}`}>
                              {sub.status}
                            </span>
              ...
              {activeTab === 'results' && testResults.length > 0 && (
                <div className="space-y-3">
                  ...
                  {testResults.map((result, idx) => (
                    <Card key={idx} className={`p-4 border-l-4 ${result.passed ? 'border-green-500' : 'border-red-500'}`}>
                      <div className="flex items-start justify-between mb-2">
                        <h4 className="text-sm font-semibold text-gray-900">Test Case {idx + 1}</h4>
                      ...
                      <div className="space-y-2">
                        ...
                        <div>
                          ...
                          <code className={`block px-3 py-2 rounded text-sm font-mono text-gray-900 ${
                        ...
                        {result.executionTime && (
                          <p className="text-xs text-gray-500">Execution: {result.executionTime}ms</p>
                        )}
                      </div>
                    </Card>
                  ))}
