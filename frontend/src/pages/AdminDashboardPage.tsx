...
const defaultStats: AdminStatistics = {
  ...
  problemsByDifficulty: { Beginner: 0, Intermediate: 0, Advanced: 0 },
  aptitudeByCategory: { Quantitative: 0, Logical: 0, Verbal: 0, Technical: 0 },
};
...
const AdminDashboardPage = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        const [stats, subs, activity] = await Promise.all([
          getAdminStatistics(),
          getRecentSubmissions(5),
          getRecentStudentActivity(10),
        ]);
        setStatistics(stats);
  ...
  const quickActions = [
    { title: 'Add Student', description: 'Create new student account', icon: Users, color: 'bg-blue-600', hoverColor: 'hover:bg-blue-700', darkColor: 'dark:bg-blue-500/15 dark:border dark:border-blue-500/30', darkHoverColor: 'dark:hover:bg-blue-500/25', url: '/admin/students/new' },
    { title: 'Add Problem', description: 'Create coding problem', icon: Code2, color: 'bg-purple-600', hoverColor: 'hover:bg-purple-700', darkColor: 'dark:bg-purple-500/15 dark:border dark:border-purple-500/30', darkHoverColor: 'dark:hover:bg-purple-500/25', url: '/admin/problems' },
    { title: 'Add Question', description: 'Create aptitude question', icon: FileQuestion, color: 'bg-green-600', hoverColor: 'hover:bg-green-700', darkColor: 'dark:bg-green-500/15 dark:border dark:border-green-500/30', darkHoverColor: 'dark:hover:bg-green-500/25', url: '/admin/aptitude' },
    { title: 'View Reports', description: 'Analytics & insights', icon: BarChart3, color: 'bg-indigo-600', hoverColor: 'hover:bg-indigo-700', darkColor: 'dark:bg-indigo-500/15 dark:border dark:border-indigo-500/30', darkHoverColor: 'dark:hover:bg-indigo-500/25', url: '/admin/reports' },
  ];

  const getSubmissionStatusIcon = (status: string) => {
    switch (status.toLowerCase()) {
      case 'accepted': case 'approved': return <CheckCircle className="w-4 h-4 text-green-600" />;
      case 'pending': return <Clock className="w-4 h-4 text-orange-500" />;
      case 'rejected': case 'wrong answer': return <XCircle className="w-4 h-4 text-red-600" />;
  ...
  const getStatusColor = (status: string) => {
    switch (status.toLowerCase()) {
      case 'accepted': case 'approved': return 'text-green-600 dark:text-green-400';
      case 'pending': return 'text-orange-600 dark:text-orange-400';
      case 'rejected': case 'wrong answer': return 'text-red-600 dark:text-red-400';
      default: return 'text-gray-600 dark:text-lc-text-muted';
    }
  };
    ...
    <Container size="xl" fullHeight>
            ...
            <Card key={index} className={`bg-gradient-to-br ${gradients[index]} hover:shadow-xl hover:scale-105 transition-all duration-300`}>
              <div className="flex items-start justify-between mb-4">
                <div className="flex-1">
                  ...
                  <p className={`text-xs font-medium ${stat.changeType === 'positive' ? 'text-green-600 dark:text-green-400' : stat.changeType === 'warning' ? 'text-orange-600 dark:text-orange-400' : 'text-gray-500 dark:text-lc-text-muted'}`}>
                    {stat.change}
                  </p>
                </div>
              ...
              <a href={stat.linkUrl} className="text-xs text-primary-600 dark:text-accent-400 hover:text-primary-700 dark:hover:text-accent-300 font-semibold flex items-center gap-1 group">
                {stat.linkText}
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {quickActions.map((action, index) => {
            ...
            return (
              <a key={index} href={action.url} className={`${action.color} ${action.hoverColor} ${action.darkColor} ${action.darkHoverColor} text-white dark:text-lc-text p-5 rounded-xl transition-all duration-300 group hover:shadow-xl hover:scale-105`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-6">
          ...
          <div className="space-y-3">
            <div>
              <div className="flex items-center justify-between mb-1">
                ...
                <span className="text-xs font-bold text-green-600 dark:text-green-400">{statistics.activeStudents}/{statistics.totalStudents}</span>
              </div>
              <div className="h-2.5 bg-gray-200 rounded-full overflow-hidden shadow-inner">
                <div className="h-full bg-gradient-to-r from-green-500 to-emerald-600 rounded-full shadow-sm transition-all duration-500" style={{ width: `${statistics.totalStudents > 0 ? (statistics.activeStudents / statistics.totalStudents) * 100 : 0}%` }} />
              </div>
            </div>
            <div>
              <div className="flex items-center justify-between mb-1">
        ...
        <Card className="p-5 bg-gradient-to-br from-purple-50 to-pink-50 border-purple-200 dark:from-purple-900/20 dark:to-pink-900/20 dark:border-purple-700 hover:shadow-xl transition-all duration-300">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-xs text-gray-600 dark:text-lc-text-muted font-semibold">Beginner</span>
              <span className="text-sm font-bold text-purple-600 dark:text-purple-400">{statistics.problemsByDifficulty.Beginner}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-xs text-gray-600 dark:text-lc-text-muted font-semibold">Intermediate</span>
              <span className="text-sm font-bold text-purple-600 dark:text-purple-400">{statistics.problemsByDifficulty.Intermediate}</span>
            </div>
            <div className="flex items-center justify-between">
              <span className="text-xs text-gray-600 dark:text-lc-text-muted font-semibold">Advanced</span>
              <span className="text-sm font-bold text-purple-600 dark:text-purple-400">{statistics.problemsByDifficulty.Advanced}</span>
            </div>
          </div>
        </Card>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card className="p-5 bg-gradient-to-br from-blue-50 to-indigo-50 border-blue-200 dark:from-blue-900/20 dark:to-indigo-900/20 dark:border-blue-700">
          <div className="flex items-center justify-between mb-4">
            ...
            <a href="/admin/submissions" className="text-xs text-blue-600 hover:text-blue-700 font-semibold hover:underline transition-all">View All →</a>
          </div>
          <div className="space-y-2 max-h-[300px] overflow-y-auto">
            {recentSubmissions.length === 0 ? (
              <p className="text-sm text-gray-500 dark:text-lc-text-muted text-center py-4">No recent submissions</p>
            ) : (
              recentSubmissions.map((submission: RecentSubmission) => (
                <div key={submission.id} className="flex items-center gap-3 p-3 bg-white dark:bg-lc-card hover:bg-blue-50 dark:hover:bg-lc-elevated rounded-lg transition-all duration-200 border border-blue-100 dark:border-lc-border-light hover:shadow-md">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      <span className={`text-xs capitalize font-semibold ${getStatusColor(submission.status)}`}>{submission.status}</span>
                    </div>
                  </div>
        ...
        <Card className="p-5 bg-gradient-to-br from-purple-50 to-pink-50 border-purple-200 dark:from-purple-900/20 dark:to-pink-900/20 dark:border-purple-700">
          <div className="flex items-center justify-between mb-4">
            ...
            <a href="/admin/students" className="text-xs text-purple-600 hover:text-purple-700 font-semibold hover:underline transition-all">View All →</a>
          </div>
          <div className="space-y-2 max-h-[300px] overflow-y-auto">
            {recentActivity.length === 0 ? (
              <p className="text-sm text-gray-500 dark:text-lc-text-muted text-center py-4">No recent activity</p>
            ) : (
              recentActivity.map((activity: StudentActivity) => (
                <div key={activity.id} className="flex items-center gap-3 p-3 hover:bg-gray-50 dark:hover:bg-lc-elevated rounded-lg transition-colors">
                  <div className={`${activity.type === 'coding' ? 'bg-blue-50 dark:bg-blue-900/30' : activity.type === 'aptitude' ? 'bg-green-50 dark:bg-green-900/30' : 'bg-purple-50 dark:bg-purple-900/30'} p-2 rounded`}>
